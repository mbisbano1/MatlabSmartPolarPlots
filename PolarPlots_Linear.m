...
% Global Variable Declarations (Default):
    ...
    PlotRadius.YMin = 0.5;                                                      % User Defineable
    ...
    PlotRadius.Labels_dB = [0, -10, -20, -30, -40];
    PlotRadius.OneSidedBWs = [60, 90, 120, 150, 180];                           % User Defineable
...
% Make the Figure and Plot the axes:
    ...
    ax.YTickMode = 'manual';
    ax.YTick = [0 flip(PlotRadius.Labels)];
    ...
    for k = 1:length(PlotRadius.Labels)
        ...
        if strcmp(ScaleType, 'dB')
            ...
            if strcmp('origin',ax.YAxisLocation)
                labelsText = text(0.01, 0.005 + (PlotRadius.Labels(k)), 0.1, num2str(PlotRadius.Labels(k)));
            end                
            if strcmp('right',ax.YAxisLocation)
                labelsText = text(1, 0.002 + (PlotRadius.Labels(k)), 0.1, num2str(PlotRadius.Labels(k)));    
            end
            if strcmp('left',ax.YAxisLocation)
                labelsText = text(-1.1, 0.002 + (PlotRadius.Labels(k)), 0.1, num2str(PlotRadius.Labels(k)));    
            end
            labelsText.Color = 'k';
